...
function createDatabase(db) {
  ...
  //cnt = await queryCount(db,'sqlite_master',` WHERE type='table' AND upper(name)=upper('${tableName_TBL_FS}') `).then( (ret)=> {console.log("cnt aa = " +ret);} );
  //cnt = queryCount(db,'sqlite_master',` WHERE type='table' AND upper(name)=upper('${tableName_TBL_FS}') `).then( (ret)=> {console.log("ret aa = " +JSON.stringify(ret));} );
  /*
  cnt.then(
  ...
  */
  ...
  /*
  const tableCheckQuery = ` SELECT COUNT(*) cnt FROM sqlite_master  WHERE type='table' AND upper(name)=upper('${tableName_TBL_FS}') `;
